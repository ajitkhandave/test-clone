...
update `reports_query_details` set `query_detail`='SELECT TRIM(pv.print_vendor) AS print_vendor ,COUNT(DISTINCT po.id) AS ordercount ,COUNT(DISTINCT (CASE WHEN LOWER(li.customer_product_id) LIKE ''%kit%'' THEN li.id END)) AS kitcount ,SUM(lic.quantity) AS quantity FROM ${schema}.platform_order po JOIN ${schema}.line_item li ON po.id = li.order_id JOIN ${schema}.line_item_component lic ON li.id = lic.line_item_id LEFT JOIN ${schema}.print_vendor pv ON pv.id = li.print_vendor WHERE po.program_id = 3 GROUP BY TRIM(pv.print_vendor)' , `datasource` = '${schema}', `created_date` = NOW(), `modified_date` = NOW()
